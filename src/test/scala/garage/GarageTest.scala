...
class GarageTest extends BaseTestingClass {
  ...
  "Garage repairs when the employee  has a workrate of 2 and the damage equals 50" should "should return 25 hours" in {
  ...
  "Garage repairs when the employee  has a workrate of 5 and the damage equals 50" should "should return 10 hours" in {
    val testEmployee = new Employee("Ralph","BossMan", 5)
    val testVehicle = new Car("Land Rover", 4, "silver", 700, Calendar.getInstance().getTime, 4, 50)
    assert(garage.fixVehicle(testVehicle, testEmployee) == 10)
  }

  "Garage repairs when the employee  has a workrate of 5 and the damage equals -5" should "should return 0 hours" in {
    val testEmployee = new Employee("Ralph","BossMan", 5)
  ...
  "Garage repair costs for 10 hours of work" should "should return £500" in {
    val testEmployee = new Employee("Ralph","BossMan", 5)
    val testVehicle = new Car("Land Rover", 4, "silver", 700, Calendar.getInstance().getTime, 4, 50)
    assert(garage.calculateBills(garage.fixVehicle(testVehicle, testEmployee)) == 500)
  }

  "Garage repair costs for 25 hours of work" should "should return £1250" in {
    val testEmployee = new Employee("Dave","Testee", 2)
    val testVehicle = new Car("Land Rover", 4, "silver", 700, Calendar.getInstance().getTime, 4, 50)
